...
extension StringLocalization on String {
  String tr({Map<String, String>? namedArgs}) {
    return LocalizationHelper.tr(this, namedArgs: namedArgs);
  }
}
...
class AppException implements Exception {
  ...
  factory AppException.httpError(int statusCode, {String? message, String? requestId}) {
    return AppException._internal(
      code: statusCode,
      message: message ?? _httpErrorMessage(statusCode),
      requestId: requestId,
    );
  }
  ...
  bool get shouldRetry {
    ...
    if (isClientError) {
      return code == 408 || code == 429; // 只重试超时和频率限制错误
    }
